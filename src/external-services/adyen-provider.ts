...
export const SERVICE_NAME = "adyen";

export function createAdyenProvider(chaos: ChaosController) {
  return {
    async charge(amount: Cents, customerId: string): Promise<Result<ChargeAdapterResult, ProviderError>> {
      await chaos.simulateLatency(SERVICE_NAME, 50, 220);
      if (chaos.shouldFail(SERVICE_NAME)) {
        const code = randomDeclineCode();
        return err(new ProviderError(
          `Adyen: charge failed for customer ${customerId} (${code})`,
          SERVICE_NAME,
          code,
        ));
      }
      return ok({
        transactionId: `adyen_txn_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
        amount,
    ...
    async refund(transactionId: string, amount: Cents): Promise<Result<RefundAdapterResult, ProviderError>> {
      await chaos.simulateLatency(SERVICE_NAME, 50, 180);
      if (Math.random() < chaos.getFailureRate(SERVICE_NAME) * 0.5) {
        return err(new ProviderError(
          `Adyen: refund failed for ${transactionId}`,
          SERVICE_NAME,
          "refund_failed",
        ));
      }
      return ok({
        transactionId: `adyen_ref_${transactionId}`,
        amount,
...
export type AdyenProvider = ReturnType<typeof createAdyenProvider>;
