...
export const SERVICE_NAME = "paypal";

export function createPayPalProvider(chaos: ChaosController) {
  return {
    async charge(amount: Cents, customerId: string): Promise<Result<ChargeAdapterResult, ProviderError>> {
      await chaos.simulateLatency(SERVICE_NAME, 60, 250);
      if (chaos.shouldFail(SERVICE_NAME)) {
        const code = randomDeclineCode();
        return err(new ProviderError(
          `PayPal: charge failed for customer ${customerId} (${code})`,
          SERVICE_NAME,
          code,
        ));
      }
    ...
    async refund(transactionId: string, amount: Cents): Promise<Result<RefundAdapterResult, ProviderError>> {
      await chaos.simulateLatency(SERVICE_NAME, 60, 200);
      if (Math.random() < chaos.getFailureRate(SERVICE_NAME) * 0.5) {
        return err(new ProviderError(
          `PayPal: refund failed for ${transactionId}`,
          SERVICE_NAME,
          "refund_failed",
        ));
      }
      return ok({
        transactionId: `paypal_ref_${transactionId}`,
        amount,
...
export type PayPalProvider = ReturnType<typeof createPayPalProvider>;
